...
/*
	...
	open for reading.
*/
static int suss_fifo( mcl_ctx_t* ctx, int mtype, int io_dir ) {
	fifo_t* fifo;
	void*	hash;
...
static int fifo_read1( void *vctx, int mtype, char* ubuf, int ublen, int long_hdrs, char* timestamp ) {
	...
	if( (fd = suss_fifo( ctx, mtype, READER ))  >= 0 )  {
		if( long_hdrs ) {
			read_header( fd, wbuf );
...
extern void mcl_fifo_fanout( void* vctx, int report, int long_hdr  ) {
	...
	while( 1 ) {
		...
		if( mbuf != NULL && mbuf->state == RMR_OK && mbuf->len > 0  ) {
			fd = suss_fifo( ctx, mbuf->mtype, WRITER );		// map the message type to an open fd
			if( fd >= 0 ) {
				if( long_hdr ) {
		...
		if( report ) {
			if( (now = time( NULL ) ) > next_report ) {
				fprintf( stdout, "[STAT] (mcl) %lld total messages written; %ld written, %ld dropped  %ld errors over last ~%d seconds\n", total, count, drops, errors, report );
